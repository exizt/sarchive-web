...
<script>
var settings = {width:482,height:390};
var itemList = [
	{name:"침대",width:150,height:200,top:171,left:20,rotate:0}
	,{name:"TV대",width:150,height:47,top:20,left:20,rotate:0}
	,{name:"책상-석훈",width:140,height:60,top:243,left:172,rotate:0}
	,{name:"책상-민진",width:60,height:180,top:20,left:309,rotate:0}
];
var selectedIndex = 0;
$(document).ready(function(){
	$("#containment-wrapper").width(settings.width);
	$("#containment-wrapper").height(settings.height);
...
function drawFurniture(list)
{
	...
	for(var i in list)
	{
		...
		var innerHtml = item.name + ' [top:'+item.top+' left:'+item.left+']';
		var html = '<div class="ui-widget-content furniture-item" style="width:'+item.width+'px;height:'+item.height+'px;top:'+item.top+'px;left:'+item.left+'px;" data-index="'+i+'">'+innerHtml+'</div>';
		var test = $(html);
...
function onDragFurniture()
{
	$(".furniture-item").draggable({
		...
		drag : function(){
			...
			selectedIndex = $(this).attr("data-index");
			itemList[selectedIndex].top = top;
			itemList[selectedIndex].left = left;
			viewItemDetail();
			//console.log("top:"+top+"  left:"+left);
			$(this).text(itemList[selectedIndex].name+" [top:"+top+" left:"+left+"]");
		}
	});
}
...
#containment-wrapper {
	...
	margin: 0 auto;
	float: left;
	background-image: url("/assets/images/uploads/room-background-2018.png");
	position: relative;
}
									...
									<div class="card">
										...
										<div class="card-block">
											<fieldset disabled id="item_fieldset">
												...
												<button type="button" class="btn btn-danger" id="delete_selected">이 데이터 삭제</button>
											</fieldset>
										</div>
									</div>
								</div>
